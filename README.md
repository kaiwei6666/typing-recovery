...
修復忘記切換注音輸入法時打出的英文鍵位。目前完成英文鍵位轉注音、音節解析、離線字詞候選與排序、保守的誤打偵測提示，以及 Tab 接受首選。尚未做完整語意理解或混合文字的自動區段修正。
...
4. 在搜尋框輸入 `su3cl3`，游標留在末端，停約半秒會出現「想輸入『你好』？」提示。按 Tab 或點「套用（Tab）」直接接受，也可點「查看候選」逐字調整。
...
- 必須聚焦原輸入框、沒有反白，且游標位於文字末端。移動游標或反白時會隱藏提示；只修復部分文字仍使用原有 `Ctrl+Shift+U` 選取流程。
...
本版仍只對整個欄位提供自動提示。接受前請查看建議，偵測器與排序器不保證判斷正確。
...
接下來處理混合文字中的疑似誤打區段，同時依實際使用的誤報／漏報案例調整偵測。音節切分歧義、範圍外上下文排序、個人化仍待後續處理；所有替換仍需使用者明確操作。
